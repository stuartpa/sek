---
title: Cord support matrix
description: Supported, conditional, parsed-only, and unsupported Cord surfaces in the current SpecExplorerKit implementation.
---

# Cord support matrix

The lexer, parser, semantic phase, explorer, and replay implementation define SEK's Cord behavior.
Legacy Spec Explorer documentation and successful parsing alone do not prove runtime support.

## Status definitions

- **Supported** — parsed and consumed with tested runtime semantics.
- **Conditional** — usable only with the stated restrictions and outcome verification.
- **Parsed-only** — accepted syntax without useful runtime semantics; do not author it.
- **Unsupported** — rejected or swallowed without usable semantics; do not author it.

## Matrix

| Surface | Status | Authoring rule |
|---|---|---|
| Config/machine declarations and inheritance | Supported | Reject unknown or cyclic bases |
| `using` name resolution | Parsed-only | Keep only for readability |
...
| `public/internal/static/abstract` modifiers | Parsed-only | Cosmetic compatibility only |
| `exclude` | Parsed-only/unsafe | Never author |
| Config-level `action event` | Conditional | Tagged, but replay is direct invocation |
| Config/behavior `call` and `return` kinds | Parsed-only | Do not rely on separate call/return semantics |
| `Condition.In` | Supported | Use finite domains and inspect emitted values |
| `Condition.IsTrue` | Conditional | Prefer simple supported expressions; verify rows |
| Interaction/Pairwise/Expand/Isolated/Seeded | Supported/Conditional | Strongest on scalar/struct paths |
| `Probability.IsTrue` + `RandomSeed` | Conditional | Both branches are unioned; seed orders, not samples |
...
| `_`, `...`, `!` | Conditional | Requires a known alphabet; negation is atomic-target only |
| `->` and binary `&` | Supported lowering | Understand context and binary-block semantics |
| Root/current `||`, `|||`, `|?|` | Conditional | Keep at root; verify same-label interleaving |
| Parallel nested beneath other algebra | Unsupported behavior | Can become empty; never author |
| Argument pinning and per-argument `_` | Supported | A non-empty argument list pins |
...
| `: fail` in a model slice | Conditional | Inspect fail metadata; command exit can still succeed |
| Direct model construct | Supported | Preferred core machine |
| Model-backed accepting paths | Supported | Verify a non-empty accepted result |
| Pure-behavior construct targets | Unsupported/unsafe | Explore pure behavior directly |
| Bounded exploration | Conditional | `PathDepth` is the reliable option |
...
| Accept-completion | Conditional | Acceptance pruning; completer semantics incomplete |
| Requirement coverage | Conditional | Aggregate report, not directed generation |
| Model return value in graph | Supported | Reflected model result is recorded |
| `/` return binding during slicing | Conditional | Model-time dataflow only |
...
4. Verify constraints affect emitted arguments; unknown or unrecognized constraints can be dropped.
...
11. Do not infer per-input rejection coverage from one action-level negative edge.
...
- [Cord language reference](cord-language.md)
