...
## SEK authoring authorities

Before authoring the model or Cord, load the installed SEK skills:
...
- `.specify/extensions/sek/skills/sek-cord-authoring/references/support-and-safety.md`

In the SEK source repository, the Cord authority is
`extensions/spec-kit-sek/skills/sek-cord-authoring/SKILL.md` and the downstream authority is
`extensions/spec-kit-sek/skills/using-sek-to-generate-tests/SKILL.md`. If an authority is
missing, stop; do not reconstruct SEK behavior from a consumer product or legacy Spec Explorer
documentation.
...
5. **Author Cord.** Create a `Config.cord` with a base `config` that declares the
   ...
   Keep this direct unsliced machine when adding focused scenario slices so negative
   conformance remains model-derived.

6. **Scaffold the project.** Create a net8 SDK-style `Model.csproj` referencing the
   `SpecExplorerKit.Modeling` package, and a `.specexplorerkit/config.json` pointing at the
